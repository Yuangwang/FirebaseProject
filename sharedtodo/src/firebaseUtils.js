...
import { getStorage, deleteObject, updateMetadata, getDownloadURL, uploadBytesResumable, ref as sRef } from "firebase/storage";
...
export function getUsersViewableListsFirebase(db, uid, setUsersLists, setIsloading) {
...
export function getCurrentListItemsFirebase(db, listKey, setItems, setIsloading) {
...
export function updateAccessorFirebase(db, storage, email, items, listKey, listName) {
...
export function getImageFirebase(storage, itemKey) {
    const imageRef = sRef(storage, `images/${itemKey}`);
...
export function uploadImageFirebase(db, storage, listKey, itemKey, file, setUploadProgress) {
    const accessorsRef = ref(db, `lists/${listKey}/accessors`);
    onValue(accessorsRef, (snapshot) => {
        const data = snapshot.val();
